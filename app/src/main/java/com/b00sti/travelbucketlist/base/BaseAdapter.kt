...
class AdapterViewModel<V : Any>(val item: V) : BaseVM<AdapterNavigator<V>>() {
    fun onItemClicked() = getNavigator().onItemClicked(item)
}

abstract class BaseAdapter<T : Any, R : BaseVH<*, *>>(ITEM_CALLBACK: DiffUtil.ItemCallback<T>) : ListAdapter<T, R>(ITEM_CALLBACK) {
