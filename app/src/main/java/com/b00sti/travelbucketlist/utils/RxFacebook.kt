 ...
 * https://github.com/saantiaguilera/android-api-RxFacebook
 *
 */
class RxFacebook private constructor() {
    ...
    /**
     * Graph path to use.
     * If using a specific domain method (eg requesting my selectedUser (ME)) this param is ignored
     ...
     */
    @JvmOverloads
    fun request(request: GraphRequest = GraphRequest()): Observable<GraphResponse> {
...
//                .onErrorResumeNext(Observable.error(IOException("Rx Facebook Error")))
                    .subscribeBy(onNext = { graphResponse ->
                        responseSubject.onNext(graphResponse).also { responseSubject.onComplete() }
                    },
                            onError = { responseSubject.onError(it).also { responseSubject.onComplete() } }
                    )
        }
